...
SLEEP_INTERVAL = 1
MONITOR_INTERVAL = 10
TRADES_FILE = './src/backtest/h4_direction/trades.csv'
TIMEFRAME = '4h'

MODEL_PATH = './src/backtest/h4_direction/model.h5'
model = load_model(model_path=MODEL_PATH)

SCALER_PATH = './src/backtest/h4_direction/scaler.pkl'
...
LOG_FILE_PATH = "./src/backtest/h4_direction/possible_next_up.log"
existing_logs = set()
    ...
    if os.path.exists(LOG_FILE_PATH):
        with open(LOG_FILE_PATH, "r") as log_file:
            for line in log_file:
                entry = line.strip().split(" proba:")[0]
                existing_logs.add(entry)

def log_error(message):
    print(f"{datetime.datetime.now()} - ERROR - {message}\n")
    with open("./src/backtest/h4_direction/trading_errors.log", "a") as log_file:
        log_file.write(f"{datetime.datetime.now()} - ERROR - {message}\n")

def log_possible_next_up(coin, opening_candle_date, probability):
    log_entry = f"{coin} opening candle: {opening_candle_date}"
    
    if log_entry not in existing_logs:
        with open(LOG_FILE_PATH, "a") as log_file:
            log_file.write(f"{log_entry} proba: {round(probability, 2)}\n")
        existing_logs.add(log_entry)
